...
// --- Funções de hash e manipulação de URL ---
...
void encurtarURL_CGI(const char *urlCompleta) {
    ...
    if (urlExistente && strcmp(urlExistente, urlCompleta) == 0) {
        ...
        printf("Content-Type: text/html\r\n\r\n");
        printf("<html><body>");
        printf("<p>URL já encurtada: <a href=\"%s%s\">%s%s</a></p>", BASE_URL, code, BASE_URL, code);
        printf("</body></html>");
        return;
    }
    ...
    printf("Content-Type: text/html\r\n\r\n");
    printf("<html><body>");
    printf("<p>URL encurtada: <a href=\"%s%s\">%s%s</a></p>", BASE_URL, code, BASE_URL, code);
    printf("</body></html>");
}
